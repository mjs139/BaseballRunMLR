---
title: "Baseball Seasons Team Aggregate"
...
count_ball_err = function(a3) {
  ...
  g_min = min(a3$game) #the game value for the first game looked at
  g_max = max(a3$game) #the game value for the last game looked at
  ...
  for(h in g_min:g_max) {
    ...
    game = subset(a3, game_id == h) #only looking at one game in a3
    n = nrow(game) #the number of rows in game
...
##Something is wrong with this code. Adds plays from each game and then repeats. 
...
#errors_per_play = err_play(tracker, errs, plays)#might need to change this

#creates the data fram that will track plays and errors for every game
#plays_data = data.frame(inn, TvsB, half_inn, half_inn_rake, plays, errs, errors_per_play)
#plays_data = data.frame(tracker, plays, errs, errors_per_play)
...
plot(err_per_play ~ half_inn, data = agg_data)
